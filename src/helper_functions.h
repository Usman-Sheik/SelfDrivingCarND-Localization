...
inline double get_gaussian_noise(double mean, double std_dev) {
  std::default_random_engine random_generator_engine{};
  std::normal_distribution<double> gaussian_distribution{mean, std_dev};
  return gaussian_distribution(random_generator_engine);
}

inline double distance_between_landmarks(const LandmarkObs &landmark_a,
                                         const LandmarkObs &landmark_b) {
  const double x_component{(landmark_b.x - landmark_a.x)};
  const double y_component{(landmark_b.y - landmark_a.y)};
  return std::sqrt(x_component * x_component + y_component * y_component);
}

inline double calculate_weight(double observation_x, double observation_y,
                               double predicted_x, double predicted_y,
                               double std_dev_x, double std_dev_y) {
  double constant{1 / std::sqrt(2 * M_PI * std_dev_x * std_dev_y)};
  double x_component{(std::pow(predicted_x - observation_x, 2)) /
                     (2 * std::pow(std_dev_x, 2))};
  double y_component{(std::pow(predicted_y - observation_y, 2)) /
                     (2 * std::pow(std_dev_y, 2))};
  return constant * std::exp(-(x_component + y_component));
}
